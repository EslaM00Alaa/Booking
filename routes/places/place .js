...
router.get("/places/:city/:limit/:pageN", async (req, res) => {
  try {
      let size = req.params.limit;
      let pageN = req.params.pageN;
      let city = req.params.city;

      let result = await client.query("SELECT p.name, p.address, p.description, p.hour_salary, pi.image FROM places p LEFT JOIN places_images pi ON p.id = pi.place_id WHERE p.active = true AND p.city = $1 AND  LIMIT $2 OFFSET $3", [city, size, (pageN - 1) * size]);
