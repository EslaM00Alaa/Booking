  ...
  try {
    ...
    if (imagePath) {
      result = await client.query(
        `INSERT INTO users (user_name, mail, pass,role,image,active) VALUES ($1, $2, $3, $4,$5,$6) RETURNING id, role, mail;`,
...
router.post("/owner/verifycode", async (req, res) => {
  try {
...
router.post("/owner/resetpass", async (req, res) => {
...
router.delete("/owner/deleteprofile", isOwner, async (req, res) => {
  try {
...
router.put("/owner/changeprofile", photoUpload.single("image"), isOwner, async (req, res) => {
  try {
    const profileImagePath = "http://localhost:6666/images/profile.png";
    const userId = req.body.id;
